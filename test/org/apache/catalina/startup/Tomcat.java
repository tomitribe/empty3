...
//    
    ...
    // not used in recent because we have direct access but in 6.0.44 not ....
    public int getLocalPort() {
        try {
            final AbstractProtocol protocolHandler = (AbstractProtocol) getConnector().getProtocolHandler();
            final Method getEndpointMethod = AbstractProtocol.class.getDeclaredMethod("getEndpoint");
            getEndpointMethod.setAccessible(true);
            final AbstractEndpoint endpoint = (AbstractEndpoint) getEndpointMethod.invoke(protocolHandler);
            if (endpoint instanceof JIoEndpoint) {
                final Field serverSocketField = endpoint.getClass().getDeclaredField("serverSocket");
                serverSocketField.setAccessible(true);
                final ServerSocket serverSocket = (ServerSocket) serverSocketField.get(endpoint);
                return serverSocket.getLocalPort();

            } else if (endpoint instanceof NioEndpoint) {
                final Field serverSocketField = endpoint.getClass().getDeclaredField("serverSock");
                serverSocketField.setAccessible(true);
                final ServerSocketChannel serverSocket = (ServerSocketChannel) serverSocketField.get(endpoint);
                return serverSocket.socket().getLocalPort();
                // ((ServerSocketChannel) ((NioEndpoint) endpoint).serverSock).localAddress().getPort()
...
//        silence(host, contextPath);
        ...
        // hack because this version of Tomcat 6 does not support null baseDir
        // No file system docBase required
        try {
            final File output = new File("output");
            File file = File.createTempFile("docBase", "", output);
            file.delete();
            file.mkdir();
            file.deleteOnExit(); // should be empty. It's just to make tomcat happy in this version
            ctx.setDocBase(file.getAbsolutePath());
