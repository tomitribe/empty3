...
import org.apache.tomcat.util.http.parser.HttpParser.SkipConstantResult;
...
public class TokenList {
    ...
    public static boolean parseTokenList(Reader input, Collection<String> collection) throws IOException {
        ...
        do {
            ...
            SkipConstantResult skipResult = HttpParser.skipConstant(input, ",");
            if (skipResult == SkipConstantResult.EOF) {
                // EOF
                valid = true;
                collection.add(fieldName.toLowerCase(Locale.ENGLISH));
                break;
            } else if (skipResult == SkipConstantResult.FOUND) {
                valid = true;
                collection.add(fieldName.toLowerCase(Locale.ENGLISH));
                continue;
            } else {
